...
class TTYSession:
    def __init__(self, cmd, *, cwd=None, env=None, encoding="utf-8", echo=False, cols=120, rows=32):
    ...
    async def start(self):
        ...
        if _IS_WIN:
            self._proc = await _spawn_winpty(
                self.cmd, self.cwd, self.env, self.echo, self.cols, self.rows
            )  # ← pass echo
        else:
    ...
    async def resize(self, cols: int, rows: int):
        ...
        self.cols = max(2, int(cols))
        self.rows = max(1, int(rows))
        resize = getattr(self._proc, "resize", None)
        if callable(resize):
            await resize(self.cols, self.rows)
...
async def _spawn_posix_pty(cmd, cwd, env, echo, cols, rows):
    import pty, asyncio, os, termios, fcntl, struct
    ...
    async def _resize(next_cols, next_rows):
        fcntl.ioctl(
            master,
            termios.TIOCSWINSZ,
            struct.pack("HHHH", max(1, int(next_rows)), max(2, int(next_cols)), 0, 0),
        )

    proc.stdin = _Stdin()  # type: ignore
    proc.stdout = reader
    proc.resize = _resize  # type: ignore[attr-defined]
    return proc
...
async def _spawn_winpty(cmd, cwd, env, echo, cols, rows):
    ...
    class _Proc:
        ...
        async def resize(self, next_cols, next_rows):
            if child.isalive():
                child.setwinsize(max(1, int(next_rows)), max(2, int(next_cols)))
