...
const model = {
  ...
  async openTerminal() {
    try {
      if (terminalStore.isModalOpen) return;
      let folderPath = "$WORK_DIR";
      try {
        const resp = await shortcuts.callJsonApi("/chat_files_path_get", {
          ctxid: shortcuts.getCurrentContextId(),
        });
        if (resp?.ok && resp.path) {
          folderPath = resp.path;
        }
      } catch (_e) {
        console.warn("Falling back to default terminal workdir");
      }

      terminalStore.prepare({
        ctxid: shortcuts.getCurrentContextId(),
        folderPath,
      });
      await openModal("modals/terminal/terminal.html");
    } catch (e) {
      if (globalThis.toastFetchError) {
        globalThis.toastFetchError("Error opening terminal", e);
      }
    }
  },
